...
// package.json dependencies (Critical for Stability)
{
  ...
  "@payloadcms/storage-vercel-blob": "3.75.0" // PINNED to match core
}
```
...
**Critical Strategy**: Strict Version Pinning.
- **Problem**: `@payloadcms/storage-vercel-blob` versions >3.75.0 introduced breaking changes (UploadHandlersProvider context) when used with Payload 3.75.0.
- **Solution**: Install `@payloadcms/storage-vercel-blob@3.75.0` exactly.
- **Config**:
  ```typescript
  ...
  ```
- **Next.js Config**: Whitelist `public.blob.vercel-storage.com` in `remotePatterns`.
...
### 🔴 Issue 3: Admin Panel Client-Side Exception (Import Map)
**Symptoms**: "PayloadComponent not found in importMap" on `/admin` after adding storage plugin.
**Cause**: The Admin UI components need to be regenerated when plugins injects new UI slots.
**Solution**:
1. Run `npm run generate:importmap`.
...
2. **Better Solution (Implemented)**: Migrate to **Neon** which handles connection pooling server-side.
...
3. `/collections` fetches data from Neon.
4. Images upload to Vercel Blob.
5. Deployment to Vercel builds successfully.
